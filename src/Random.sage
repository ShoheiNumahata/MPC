...
shQ = [0 for i in range(len(Q))]
for i in range(len(Q)):
    exec_command = ('shQ[' +str(i) + '] = P'+ str(i + 1) +  '.share_r2' )
    exec(exec_command)
...
#プロトコルの確認作業
r2 = reconst(la_list,shQ)

#ｒが正しくshare_rでシェアが生成されているか
Ar = K(P1.s1 + P2.s1 +P3.s1)

Ar2 = K(pow(Ar,2))
Ashare_r = (P1.share_r,P2.share_r,P3.share_r)
Ar_reconst_r = reconst(la_list,Ashare_r) 
